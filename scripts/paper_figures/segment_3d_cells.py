...
* The C++ uses ``std::random_device`` and is therefore not
  reproducible from Python (B8). To make this script deterministic
  the Python wrapper here samples the seed candidates with
  ``numpy.random.default_rng(seed)`` and passes them as
  ``seed_points`` to the C++ kernel, which disables the C++ random
  draw entirely.
